...
from stockticker.ingest.wikipedia.sync import (
    MAX_COMPANY_DEACTIVATIONS_PER_RUN,
...
async def test_missing_once_stays_active_missing_twice_is_deactivated(conn: AsyncConnection) -> None:
    await apply_constituents(conn, ROWS)
    gone = {"MMM", "AOS"}

    first = await apply_constituents(conn, _without(gone))
    assert gone <= await _active(conn, "listings", "symbol")
    assert first.listings_deactivated == 0

    second = await apply_constituents(conn, _without(gone))
...
async def test_reappearing_resets_the_miss_count(conn: AsyncConnection) -> None:
    await apply_constituents(conn, ROWS)
    await apply_constituents(conn, _without({"MMM"}))
    await apply_constituents(conn, ROWS)
...
async def test_a_ticker_change_moves_the_primary_and_retires_the_old_listing(conn: AsyncConnection) -> None:
    ...
    await apply_constituents(conn, renamed)
...
async def test_ten_company_deactivations_are_allowed(conn: AsyncConnection) -> None:
    await apply_constituents(conn, ROWS)
    gone = {row.symbol for row in ROWS[:MAX_COMPANY_DEACTIVATIONS_PER_RUN]}
    await apply_constituents(conn, _without(gone))

    summary = await apply_constituents(conn, _without(gone))

    assert summary.companies_deactivated == MAX_COMPANY_DEACTIVATIONS_PER_RUN


async def test_more_than_ten_company_deactivations_fail_the_run(conn: AsyncConnection) -> None:
    await apply_constituents(conn, ROWS)
    gone = {row.symbol for row in ROWS[: MAX_COMPANY_DEACTIVATIONS_PER_RUN + 1]}
    await apply_constituents(conn, _without(gone))

    with pytest.raises(TooManyDeactivationsError) as raised:
        await apply_constituents(conn, _without(gone))

    assert len(raised.value.ciks) == MAX_COMPANY_DEACTIVATIONS_PER_RUN + 1
