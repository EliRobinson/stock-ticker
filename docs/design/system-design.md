...
- Parts are validated at the boundary as a Pydantic discriminated union that mirrors the AI SDK's `UIMessagePart`: `text`, `step-start`, tool parts by `state`, and a catch-all for `data-*` and everything else. A part type the server does not know, or a known type with a bad shape, is dropped, so a newer client (or one bad persisted part) never bricks the chat.
