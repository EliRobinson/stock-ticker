...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fresh data: live Quotes and daily history  | A Python worker polls Alpaca for Quotes and backfills Daily Bars, and computes Market Cap from SEC EDGAR share counts. FastAPI serves it all read-only.                                                                   | Done: [#3](https://github.com/EliRobinson/stock-ticker/issues/3) (API/worker/schema foundation), [#5](https://github.com/EliRobinson/stock-ticker/issues/5) (S&P 500 list, EDGAR share counts and filings, Market Cap) · In review: [#4](https://github.com/EliRobinson/stock-ticker/issues/4) (Alpaca Quotes and Bars) |
| Tables and charts, with zoom, filter, sort | TanStack Table and TanStack Virtual drive the Market table; lightweight-charts drives the price chart, with range presets and a custom range picker.                                                                      | Done: [#8](https://github.com/EliRobinson/stock-ticker/issues/8) (data layer) · In review: [#9](https://github.com/EliRobinson/stock-ticker/issues/9) (screens)                                                                                                                                                         |
| Notes tied to dates and companies          | A Notes CRUD endpoint on the API; the web app renders Notes as chart markers and as a filterable list, last write wins.                                                                                                   | Done: [#6](https://github.com/EliRobinson/stock-ticker/issues/6) · In review: [#9](https://github.com/EliRobinson/stock-ticker/issues/9) (Notes screen and chart markers)                                                                                                                                               |
| AI that answers with tables and charts     | A Python tool loop runs Claude over guarded, read-only SQL and streams the AI SDK UI message protocol. The web app renders the resulting `data-view` parts with the same table and chart components used everywhere else. | Backend in review: [#7](https://github.com/EliRobinson/stock-ticker/issues/7). Web in review: [#9](https://github.com/EliRobinson/stock-ticker/issues/9) (Ask panel)                                                                                                                                                    |
...
Four screens, built from the Claude Design project's state board and rendered here from fixtures. Every state in the [brief](docs/design/brief.md) has a frame on the dev-only board at `/dev/states` (run `pnpm --filter web dev`).
...
| **Ask beside Market**                                                                                       | **Notes**                                                                                          |
| [![Ask docked beside Market, dark](docs/screenshots/ask-1440-dark.png)](docs/screenshots/ask-1440-dark.png) | [![Notes, light](docs/screenshots/notes-1440-light.png)](docs/screenshots/notes-1440-light.png)    |

More in [docs/screenshots/](docs/screenshots): each screen in light and dark at 1440px and 400px, and the state board group by group.
...
| Claude Design project                      | The UI state board (every screen and state, light and dark), implemented via the claude_design MCP                                                                                                              | [docs/screenshots/](docs/screenshots)                        |
